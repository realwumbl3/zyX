...
function calculateDominantColor(img_url) {
	return new Promise((resolve, reject) => {
		const Img = new Image(),
			blockSize = 30; // only visit every x pixels,
		const __starttimer__ = performance.now();
		...
		Img.onload = () => {
			...
			console.log("- - - hue calculate done.", Math.round(performance.now() - __starttimer__), "ms");
			resolve({ h, s, l });
		};
	});
}
