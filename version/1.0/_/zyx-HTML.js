			...
			if (customTags) {
				...
				switch (a) {
					...
					case "pr0x":
						...
						break;
					case "this":
						const splitAttrValue = attrValue.split(" ");
						if (splitAttrValue.length > 1) {
							const [first_key, second_key] = splitAttrValue;
						...
						} else if (splitAttrValue.length === 1) {
							thisNode.__key__ = attrValue;
							that.__scope__[attrValue] = thisNode;
						}
...
export function htmlLiteralToTemplate(raw, ...data) {
	...
	for (let [key, val] of Object.entries(data)) {
		if (val?.__zyxHTML__) { // html string argument is a class object binded to another scope
		...
		if (val instanceof ZyxHTML) { // parent < > child extending + push array culm
