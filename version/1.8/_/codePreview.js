...
export class CodePreview {
  constructor() {
    ...
    html` <style>
        ...
        /* Pre element with whitespace preserved but with wrapping */
        .code-preview div {
  ...
  highlightSyntax(line) {
    ...
    const tokenPatterns = [
      ...
      // Keywords
      { regex: /\b(const|let|var|function|class|if|else|for|while|return|export|import|from|async|await|instanceof|typeof|new|this|try|catch|finally|throw|break|continue|switch|case|default|of|in|extends|implements|static|get|set)\b/g, className: "keyword" },
      
      // Missing opening brackets in constructor calls - must come before other function handling
      { regex: /\bnew\s+([A-Z][a-zA-Z0-9_$]*)\s*\)/g, className: "punctuation", transform: (match) => {
      ...
      { regex: /\.([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\)/g, className: "property", transform: (match) => {
      ...
      // Constructor calls
      { regex: /\bnew\s+([A-Z][a-zA-Z0-9_$]*)/g, className: "class", transform: (match) => {
        const parts = match.match(/\bnew\s+([A-Z][a-zA-Z0-9_$]*)/);
        if (parts && parts[1]) {
      ...
      { regex: /\b([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\)/g, className: "function", transform: (match) => {
        if (match.includes('(')) return match; // Skip if it already has an opening bracket
