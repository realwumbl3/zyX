...
export function htmlLiteralToTemplate(raw, ...data) {
	...
	while (entries.length > 0) {
		...
		if (val?.__zyxHTML__ || val instanceof ZyxHTML || val instanceof HTMLElement || val instanceof DocumentFragment) {
	...
	for (const strExpPh of [...template.querySelectorAll("StrExpr")]) {
		let replacementData = data[strExpPh.id];
		if (replacementData instanceof ZyxHTML) {
			Object.assign(this.__scope__, replacementData.__scope__);
			replacementData = replacementData.__html__;
		} else if (replacementData?.__zyxHTML__) {
			replacementData = replacementData.__zyxHTML__.__html__;
		}
	...
	// if (debugModule) debugLog("html", { min: 0, 
		// dump: { raw, ...data }
	// });
	return template;
}
