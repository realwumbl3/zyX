...
export function applyZyxAttrs(that) {
	try {
		for (const [preset, func] of Object.entries(zyxAttrs)) {
			for (const e of [...that.content.querySelectorAll(`[zyx-${preset}]`)]) {
...
zyxTags["smart-placehold"] = (zyXcomponent, element) => {
	console.log('zyxTags:"smart-placehold"');
	console.log({ zyXcomponent, element });
};
