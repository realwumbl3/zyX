...
export class zyXHtml extends HTMLElement {
	...
	const() {
		...
		console.log("markup")
		console.log(...markup.childNodes)
		this.becomeOtherNode(markup.firstChild);
		// this.append(...markup.childNodes);
		this.__constructed__ = true;
		return this;
	}

	becomeOtherNode(node) {
		console.log("become", node)
		// inherit all properties, classes, attributes, and events from the node
	...
	bind(any) {
		...
		any.appendTo = (container) => container.append(this);
		any.prependTo = (container) => container.prepend(this);
		any.place = (place) => placer(this, place);
		return this.const();
	}
	...
	touch(_) {
		...
		_({ proxy: this.proxy, markup: this });
		return this;
	}
	...
	return(_) {
		return _({ this: this, proxy: this.proxy, markup: this });
	}
...
customElements.define("d-iv", zyXHtml);

function htmlLiteralProcessor(raw, ...string_data) {
	const { data, stringExpressions } = htmlLiteralDataProcessor(string_data);

	const raw_splitnt = raw.raw.map((_, i) => {
		return _.replace(/\n/g, "").replace(/\t/g, "").replace(/  /g, "");
	});

	const string = String.raw({ raw: raw_splitnt }, ...stringExpressions);
		...
		if (Array.isArray(value)) {
			...
			fragment.append(
				...value.map((item) => {
					...
					if (item instanceof zyXHtml) return item;
					if (item instanceof DoJoin) return item.zyxHTMLInstance;
...
function processPlaceholders(markup, data) {
	...
	for (const stringPlaceholder of [...markup.querySelectorAll(placehold_tag)]) {
		...
		if (placeholder_data instanceof DoJoin) {
			stringPlaceholder.replaceWith(placeholder_data.zyxHTMLInstance);
