...
export function shadowCss(name) {
	...
	if (!ZYXDATA.sr.hasOwnProperty(name)) {
		console.trace();
		return console.error("sr not found", name);
	}
	const path = ZYXDATA.sr[name];
	return html`
		<link rel="stylesheet" href="${path}" />
	`;
}

export function shadowBundleCss(name) {
	if (!window.hasOwnProperty("ZYXDATA")) {
		console.trace();
		return console.error("ZYXDATA not found");
	}
	if (!ZYXDATA.scopes.hasOwnProperty(name)) {
		console.trace();
		return console.error("scope not found", name);
	}
	const scope = ZYXDATA.scopes[name];
	return scope.map(
		(path) =>
			html`
				<link rel="stylesheet" href="${path}" />
...
export function initZyxFramework() {
	...
	window.shadowBundleCss = shadowBundleCss;
